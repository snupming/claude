 ...
 * SSCD 전처리 파이프라인.
 *
 * 순서: 이미지 디코딩 → 패딩 트리밍 → 320×320 square resize → ImageNet 정규화 → NCHW float[]
 *
 * 패딩 트리밍은 필수 전처리입니다. 트리밍 없이 SSCD를 실행하면
 * padding_black 변형에서 유사도 0.29가 나와 gap이 0.01로 붕괴합니다.
 * (v1.3 Final 섹션 5 참조)
 */
@Component
class ImagePreprocessor {
    ...
    /**
     * BufferedImage → SSCD 입력 텐서 (NCHW float[]).
     */
    float[] preprocess(BufferedImage image) {
        // Step 1: 패딩 트리밍 (필수)
        BufferedImage trimmed = trimPadding(image);

        // Step 2: 320×320 square resize (SSCD는 의도적으로 skew)
    ...
    /**
     * 단색 패딩(검정/흰색/임의 균일색) 자동 제거.
     *
     * 4개 코너 픽셀의 최빈색을 보더 색상으로 결정하고,
     * 상하좌우 각 변에서 보더 색상과 RGB 거리 30 이내인 행/열을 트리밍합니다.
     * 최소 콘텐츠 영역 10%, 5% 미만 트리밍 시 원본 반환.
     */
