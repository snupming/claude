...
| BFF 프록시 인증 (httpOnly 쿠키) | ✅ | 설계서대로 잘 구현됨 |
| 레이아웃 (default, dashboard) | ✅ | |
| **랜딩 페이지** (히어로, 기능, 프라이싱, CTA) | ✅ | 섹션 구성 충실 |
| **로그인/회원가입** | ✅ | 이메일+비밀번호 인증 동작 |
| 대시보드 기본 셸 (사이드바 + 모바일 하단 탭) | ✅ | |
| 다크모드 토글 | ✅ | 설계서에는 MVP 미구현이라 했으나 구현됨 |
| 알림 페이지 (목 데이터) | ✅ | 필터/읽음 처리 UI 존재 |
...
- **프론트엔드: B-** -- 랜딩/로그인/회원가입/대시보드 셸이 구현됨. BFF 인증 흐름 동작. 설계서 대비 composables, API 연동, 기능 페이지(이미지/탐지/증거)가 미구현
- **백엔드: D+** -- auth 모듈만 구현. 설계서에서 ✅로 표시한 image/detection/evidence 모듈이 코드에 없음. 테스트 0개
